...
export class WorkbenchEngineService {
  ...
  validateStep(step: WorkflowStep, state: WorkbenchState): { valid: boolean; error: string } {
    switch (step) {
      case 'diagnosis':
        if (!state.diagnosisResult || state.diagnosisResult.trim().length < 10) {
          return { valid: false, error: 'Please enter a diagnosis result of at least 10 characters.' };
        }
      ...
      case 'validation': {
        const unchecked = state.validationChecklist.filter(v => !v.checked);
        if (unchecked.length > 0) {
          return { valid: false, error: `${unchecked.length} validation item(s) are not checked.` };
        }
        return { valid: true, error: '' };
      }
